...
- (CGRect)caretRectForPosition:(UITextPosition *)position {
    ...
    if (originalRect.size.height >= 30) {
        originalRect.origin.y += 10;
    }
    originalRect.size.height = 20.0;
...
//    CGContextSetCharacterSpacing (context, -15);
...
- (void) drawTagImageInRect:(CGRect) rect name:(NSString*)nameText
{
    ...
    tagButton.frame = CGRectMake(rect.origin.x,
                                 ...
                                 rect.size.width,
                                 rect.size.height - 8);
    nameText = [nameText stringByReplacingOccurrencesOfString:@"_" withString:@" "];
...
//                NSLog(@"Deleted annotation tag >>>>> id(%@):range(%d,%d)",[attrs objectForKey:keyModelId], tagRange.location, tagRange.length);
                ...
                NSString *userId = [attrs objectForKey:keyModelId];
                MintAnnotation* annotation = [self annotationForId:userId];
                [self.annotationList removeObject:annotation];
                
                [[NSNotificationCenter defaultCenter] postNotificationName:@"annotation_removed" object:[attrs objectForKey:keyModelId]];
...
//    paragraphStyle.paragraphSpacing = 30.0f;
//    paragraphStyle.lineSpacing = 12;
    paragraphStyle.minimumLineHeight = 30;

    return @{NSFontAttributeName : self.font, NSParagraphStyleAttributeName : paragraphStyle};
}
