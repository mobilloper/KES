...
- (NSString *)timeAgo
{
    ...
    {
        return [self stringFromFormat:@"%%d %@seconds" withValue:deltaSeconds];
    }
    else if(deltaSeconds < 120)
    {
        return NSDateTimeAgoLocalizedStrings(@"a min");
    }
    else if (deltaMinutes < 60)
    {
        return [self stringFromFormat:@"%%d %@mins" withValue:deltaMinutes];
    }
    ...
    else if (deltaMinutes < (24 * 60))
    {
        ...
        return [self stringFromFormat:@"%%d %@hours" withValue:minutes];
    }
...
- (NSString *)differenceFromToday
{
    NSCalendar *sysCalendar = [NSCalendar currentCalendar];
    NSCalendarUnit unitFlags = NSCalendarUnitHour | NSCalendarUnitMonth | NSCalendarUnitMinute | NSCalendarUnitDay | NSCalendarUnitSecond;
    NSDateComponents *breakdownInfo = [sysCalendar components:unitFlags fromDate:[NSDate date]  toDate:self  options:0];
    
    NSInteger month = [breakdownInfo month];
    NSInteger day = [breakdownInfo day];
    NSInteger hour = [breakdownInfo hour];
    NSInteger minute = [breakdownInfo minute];
    NSInteger second = [breakdownInfo second];
    
    if (month > 0) {
        return [NSString stringWithFormat:@"%ldmonths %lddays %ldhrs %ldmins %ldsecs", (long)month, (long)day, (long)hour, (long)minute, (long)second];
    }
    if (day > 0) {
        return [NSString stringWithFormat:@"%lddays %ldhrs %ldmins %ldsecs", (long)day, (long)hour, (long)minute, (long)second];
    }
    if (hour > 0) {
        return [NSString stringWithFormat:@"%ldhrs %ldmins %ldsecs", (long)hour, (long)minute, (long)second];
    }
    if (minute > 0) {
        return [NSString stringWithFormat:@"%ldmins %ldsecs", (long)minute, (long)second];
    }
    if (second > 0) {
        return [NSString stringWithFormat:@"%ldsecs", (long)second];
    }
