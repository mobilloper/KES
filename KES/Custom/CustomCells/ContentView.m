...
{
    if ([textView.text isEqualToString:@"Write a message"]) {
        textView.text = @"";
...
{
    NSString *strComment = self.chatTextView.text;
    ...
    if (strComment.length == 0 || [strComment isEqualToString:@"Writeamessage"]) {
        self.chatTextView.text = @"Write a message";
        [self.chatTextView setTextColor:[UIColor colorWithRed:131.0f/255 green:131.0f/255 blue:131.0f/255 alpha:1.0f]];
    }
